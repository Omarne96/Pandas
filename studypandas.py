...
df=pd.read_csv("data_file.txt",delimiter="\t")  ##delimiter="\t" to separate it
''' 
        You can read a file from URL too with read_csv("URL") 
        Or you can read a table like read_table("URL")
'''
...
ans=ok.groupby(['Type 1']).mean().sort_values('Attack',ascending=False) ## sort the average from the highest attack to the low
...
okok=df.groupby('Name').Attack.agg(['min','max','count','mean']) 
